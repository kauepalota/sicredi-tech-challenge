...
class TopicController(val service : TopicService) {
    @GetMapping
    @Operation(summary = "Get all topics", description = "Get all topics from the database")
    @ApiResponse(responseCode = "200", description = "List of topics")
    fun getAll() = service.getAllTopics()

    @GetMapping("/{id}")
    @Operation(summary = "Get a topic", description = "Get a topic from the database.")
    @ApiResponse(responseCode = "200", description = "Topic found")
    @ApiResponse(responseCode = "404", description = "Topic not found")
    fun getOne(@PathVariable id: Int) = service.getTopic(id)

    @PostMapping
    @Operation(summary = "Create a topic", description = "Create a topic in the database.")
    ...
    @DeleteMapping("/{id}")
    @Operation(summary = "Delete a topic", description = "Delete a topic from the database.")
